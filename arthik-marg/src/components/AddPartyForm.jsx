...
import { User, Calendar, Plus, Upload, X } from 'lucide-react'; 
    ...
    const TabButton = ({ name }) => (
        <button
            ...
            className={`px-4 py-2 text-sm font-medium transition-colors duration-200 ${
                activeTab === name
                    ? `${CUSTOM_BLUE_TEXT} border-b-2 border-[#172554]`
                    : `text-gray-500 ${GRAY_HOVER_BG}`
            }`}
        >
            {name}
        </button>
        ...
        <button
            ...
            className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors duration-200 border ${
                transactionType === type
                    ? `${CUSTOM_BLUE} text-white border-transparent shadow-md`
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl transform transition-all">
                        ...
                        <div className="flex gap-3">
                            <button
                                onClick={() => {
                                    ...
                                    setTransactionType('To Receive'); // Auto select for Customer
                                }}
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    setTransactionType('To Give'); // Auto select for Supplier
                                }}
                                className={`px-4 py-2 text-sm font-medium rounded transition-colors duration-200 border ${
                                    partyType === 'Supplier'
                                        ? `${CUSTOM_BLUE} text-white`
                ...
                <div className="flex justify-end p-4 border-t border-gray-200 bg-gray-50 rounded-b-lg">
                    <button
                        onClick={handleSave}
                        className={`flex items-center gap-2 px-6 py-3 ${CUSTOM_BLUE} text-white rounded-lg font-semibold ${CUSTOM_BLUE_HOVER_BG} transition-colors duration-200 shadow-md`}
                    >
                        Save Party
                    </button>
                </div>
